...
const createPayment = require("../services/create_payment");
...
class VNPayController{
    ...
    async vnpayReturn (req, res, next) {
        ...
        let hmac = crypto.createHmac("sha512", secretKey);
        let signed = hmac.update(new Buffer(signData, 'utf-8')).digest("hex");     
        let booking = await getBookingById(req.query.vnp_TxnRef);
        if(secureHash === signed){
            ...
            cookies.setCookie(
                ...
                JSON.stringify(message("Bạn đã thanh toán thành công!",constantMesages.successCustom)),
                1
            );
            let payment = {
                type: "Thanh toán online qua chuyển khoản",
                is_success:"true",
                booking:booking,
            }
            ...
            cookies.setCookie(
                constants.has_message,
                JSON.stringify(message("Bạn đã thanh toán thất bại!",constantMesages.errorCustom)),
                1
            );
